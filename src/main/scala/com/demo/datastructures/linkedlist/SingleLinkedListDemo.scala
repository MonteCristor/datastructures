...
class SingleLinkedList {
  ...
  //修改
  //1. 给我一个新的结点，根据结点的no，去该对应的链表中的结点信息
  def update(heroNode: HeroNode): Unit ={
    ...
    if(flag) {
      ...
      printf("你要修改的结点no=%d 不存在~", heroNode.no)
    }
  }
  ...
  def list(): Unit = {
    ...
    breakable {
      while (true) {
        //先输出temp指向的结点信息
        printf("结点信息为no = %d name=%s nickname=%s\n", temp.no, temp.name, temp.nickName)
        //判断是否temp是不是最后一个结点
        if (temp.next == null) {
          break()
        }
  ...
  //添加结点到链表
  //第二种方式在添加英雄是，根据排名将英雄插入到指定位置（如果有这个排名 ，则添加失败，并给出提示）
    ...
    breakable{
      while (true) {
        ...
        if (temp.next.no < heroNode.no) {
          //说明heroNode 添加到temp的后面
          flag=true
          break()
        }

        if (temp.next.no < heroNode.no) {
          //说明 heroNode添加到temp的后面
          flag = true
          break()
  ...
  def add(heroNode: HeroNode): Unit = {
    //1. 先找到链表的最后结点
    //2. 先定义一个辅助变量（指针），指向head
    var temp = head
    //3. 遍历链表， 知道temp.next == null
    breakable{
      while (true) {
        if (temp.next == null) {
          //temp已经是最后的结点
